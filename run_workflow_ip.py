# IPAdapter рендер через ComfyUI: подмешивает картинку-референс из папки (ref_dir) с силой weight.
# Настройки -> config_ip.txt | Промпт -> prompt.txt | Негатив -> negative.txt
# Для КАЖДОЙ картинки из ref_dir делает count вариаций (seed инкрементится сквозным счётчиком).
# Сила референса -> weight в config_ip.txt (0.2-0.3 = мягко, 1.0 = сильно).
# Сид: первым аргументом. Картинки -> ComfyUI\output\ГГГГ-ММ-ДД\ipadapter...
...
def load_config():
    cfg = {}
    p = os.path.join(HERE, "config_ip.txt")
    if os.path.exists(p):
    ...
    for v in range(count):
        ...
        for nid, node in wf.items():
            ...
            if ct == "CheckpointLoaderSimple":
                ...
                node["inputs"]["filename_prefix"] = today + "/ipadapter"
